...
import { formatFileSize } from "@workspace/ui/lib/format";
...
const variants = {
  ...
  image:
    "border-0 p-0 w-full aspect-square relative shadow-md bg-slate-200 dark:bg-slate-900 rounded-md",
  active: "border-2",
  disabled:
        ...
        <div className="grid grid-cols-3 gap-2 sm:grid-cols-4 lg:grid-cols-5">
          {value?.map(({ file, progress }, index) => (
            <div key={index} className={variants.image}>
              ...
              {/* Remove Image Icon - Remove the progress === "PENDING" condition */}
              {imageUrls[index] && !disabled && (
                ...
                >
                  <div className="flex h-4 w-4 cursor-pointer items-center justify-center rounded-md border border-solid border-gray-500 bg-white transition-all duration-300 hover:h-5 hover:w-5 dark:border-gray-400 dark:bg-black">
                    <X
          ...
          {(!value || value.length < (dropzoneOptions?.maxFiles ?? 0)) && (
            <div {...getRootProps({ className: dropZoneClassName })}>
              ...
              <div className="flex flex-col items-center justify-center text-xs text-gray-400">
                <UploadCloudIcon className="mb-1 h-5 w-5" />
                <div className="text-gray-400 text-[10px]">drag & drop</div>
                <div className="mt-2">
