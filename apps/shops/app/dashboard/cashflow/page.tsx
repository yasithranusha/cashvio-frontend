...
const mockExpenses = [
  ...
  { id: 2, description: "Electricity & Water", amount: 8500, date: "2025-05-08" },
...
const mockRecurringPayments = [
  { id: 1, description: "Shop Rent", amount: 25000, frequency: "Monthly", nextDate: "2025-06-05" },
  { id: 2, description: "Internet", amount: 3500, frequency: "Monthly", nextDate: "2025-06-10" },
  { id: 3, description: "Insurance", amount: 4500, frequency: "Monthly", nextDate: "2025-06-15" },
  { id: 4, description: "POS System Fee", amount: 2000, frequency: "Monthly", nextDate: "2025-06-20" },
];

const mockUpcomingPayments = [
  { id: 1, description: "Staff Wages", amount: 42000, dueDate: "2025-05-15", type: "recurring", isPriority: true },
  { id: 2, description: "Supplier Invoice #1234", amount: 18000, dueDate: "2025-05-18", type: "one-time", isPriority: false },
  { id: 3, description: "VAT Payment", amount: 12500, dueDate: "2025-05-20", type: "one-time", isPriority: true },
  { id: 4, description: "Equipment Rental", amount: 7500, dueDate: "2025-05-22", type: "recurring", isPriority: false },
];
...
export default function CashFlowPage() {
  ...
  const projectedBalance = currentBalance - mockUpcomingPayments.reduce((sum, item) => sum + item.amount, 0);

  // Daily goals calculation
  const totalUpcoming = mockUpcomingPayments.reduce((sum, item) => sum + item.amount, 0);
    ...
    <div className="container mx-auto py-6 max-w-7xl">
        ...
        <div className="flex gap-2">
          ...
          <Dialog>
            ...
            <DialogContent className="sm:max-w-[425px]">
              ...
              <DialogFooter>
                <Button type="submit" onClick={handleSubmitNewPayment}>Save Transaction</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
        </div>
      ...
      {showLargePaymentAlert && (
        <Alert variant="destructive" className="mb-6">
          ...
          <AlertDescription>
            This payment amount is over 70% of your current cash on hand and may impact your ability to meet other financial obligations.
          </AlertDescription>
          <Button variant="outline" size="sm" className="mt-2" onClick={() => setShowLargePaymentAlert(false)}>
            Dismiss
          </Button>
        </Alert>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <AlertTriangle className={`h-4 w-4 ${projectedBalance < 0 ? 'text-rose-500' : 'text-muted-foreground'}`} />
          </CardHeader>
          <CardContent>
            <div className={`text-2xl font-bold ${projectedBalance < 0 ? 'text-rose-600' : ''}`}>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <div>
                ...
                <div className="text-2xl font-bold">{formatCurrency(dailyTarget * (dailyProgress / 100))}</div>
              </div>
              <div>
                <div className="text-sm font-medium">Remaining</div>
                <div className="text-2xl font-bold">{formatCurrency(dailyTarget - (dailyTarget * (dailyProgress / 100)))}</div>
              </div>
            </div>
                ...
                <CardContent>
                  <ScrollArea className="h-[320px]">
                    <div className="space-y-4">
                      ...
                      {[...mockIncome.map(item => ({...item, type: 'income'})), 
                         ...mockExpenses.map(item => ({...item, type: 'expense'}))]
                        .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
                        .map((item) => (
                          <div key={`${item.type}-${item.id}`} className="flex items-center justify-between p-2 border-b">
                            <div className="flex items-center space-x-4">
                              <div className={`${item.type === 'income' ? 'bg-emerald-100' : 'bg-rose-100'} p-2 rounded-full`}>
                                {item.type === 'income' ? (
                            ...
                            <p className={`font-medium ${item.type === 'income' ? 'text-emerald-600' : 'text-rose-600'}`}>
                  ...
                  <div key={item.id} className="flex items-center justify-between border-b pb-3">
                    <div>
                      ...
                      <div className="flex items-center mt-1">
                        ...
                        <p className="text-xs text-muted-foreground">Next: {item.nextDate}</p>
                      </div>
                    </div>
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <div className="flex items-center mt-1 space-x-2">
                            <Badge variant={payment.type === "recurring" ? "default" : "secondary"} className="text-xs">
            ...
            <CardContent>
              <form className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input id="payment-description" placeholder="Enter payment description" />
                </div>
                <div className="space-y-2">
