                    ...
                    {transaction.discount && transaction.discount > 0 && (
                      <tr>
                        ...
                        <td className="pt-1 text-right text-green-600">
                          -Rs. {transaction.discount.toLocaleString()}
                        </td>
                      </tr>
                    )}
                    {transaction.tax && transaction.tax > 0 && (
                      <tr>
                        ...
                        <td className="pt-1 text-right">
                          Rs. {transaction.tax.toLocaleString()}
                        </td>
                      </tr>
                    )}
...
const generateMockTransactions = (count: number): Transaction[] => {
  const statuses: OrderStatus[] = ["COMPLETED", "REFUNDED"];
  ...
  for (let i = 0; i < count; i++) {
    ...
    transactionDate.setDate(
      transactionDate.getDate() - Math.floor(Math.random() * 30)
    ...
    const paymentMethod =
      ...
      "Cash";
    const register =
      registers[Math.floor(Math.random() * registers.length)] || "Register 1";
    const cashier =
      cashiers[Math.floor(Math.random() * cashiers.length)] || "Unknown";
      ...
      ? {
          name:
            [
              ...
              "Chamari de Silva",
              "Lakshman Bandara",
            ][Math.floor(Math.random() * 5)] || "Customer",
          phone:
            Math.random() < 0.8
              ? `07${Math.floor(Math.random() * 10)}-${Math.floor(
    ...
    const items = Array.from(selectedProducts).map((index, i) => {
      ...
      const discount = hasDiscount
        ? Math.floor(product.price * quantity * 0.1)
        : 0;
    ...
    const discount = hasDiscount
      ? Math.floor(subTotal * (Math.random() * 0.15))
      : 0;
    ...
    const notes =
      Math.random() < 0.2
        ? [
            "Customer requested fast service",
            "Item has 3-month warranty",
